---
title: "Z$ Tokens"
description: "Understand how ZUPYies — the Zupy loyalty token — work: earning, checking balance, and using them in reward redemptions"
---
...
response = requests.post(
    ...
    json={"amount": 50, "reason": "Order #12345"},
    headers=HEADERS,
)
result = response.json()["data"]
print(f"Points added: {result['amount']}")
...
const response = await fetch(
  `${BASE_URL}/customers/2awTHloSJX7kGGprFerOOsvABcd/points/add/`,
  {
    ...
    body: JSON.stringify({ amount: 50, reason: "Order #12345" }),
  }
);
const { data: result } = await response.json();
console.log(`Points added: ${result.amount}`);
// Z$ distribution happens asynchronously in the background
```
</CodeGroup>
...
Marketing coupons (rewards with `origin_type=marketing`) are funded by the company's
...
- The **3 configurable percentages must sum to exactly `50.00`** so that customer + the
  three together equal 100% of `settlement_budget`. Any individual split can be `0` if
  the merchant chose to allocate everything to the other actors.
- **`operator_split_pct` may be the integration partner**, not the merchant — if your
  integration is the entity publishing the coupon, this is your commission. Always
  check this field for your share.
...
A marketing coupon with `settlement_budget = "100.00"`, `operator_split_pct = "0.00"`,

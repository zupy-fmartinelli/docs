...
<Note>
  **Why only four events?** The OpenDelivery `LoyaltyEventType` enum is **closed** — the standard
  ...
  Zupy's [native webhook surface](/guides/outbound-webhooks) carries **nine** event types. The five
  without an OpenDelivery counterpart — `customer.enrolled`, `points.spent`, `coupon.cancelled`,
  `coupon.expiring`, `coupon.expired` — are only available there (or via the real-time read
  endpoints in §4, which always reflect current state). The mapping:

  | Native event | OpenDelivery `eventType` |
  | --- | --- |
  | `points.earned` | `loyalty.points.earned` |
  | `reward.redeemed` | `loyalty.redemption.created` |
  | `coupon.used` | `loyalty.coupon.used` |
  | `customer.tier_changed` | `loyalty.tier.changed` |
  | `customer.enrolled`, `points.spent`, `coupon.cancelled`, `coupon.expiring`, `coupon.expired` | — no OD counterpart (native surface only) |
</Note>
